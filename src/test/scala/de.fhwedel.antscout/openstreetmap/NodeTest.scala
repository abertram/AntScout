...
class NodeTest extends FunSuite with ShouldMatchers {
    test("Node.distanceTo, Fasanenweg") {
    ...
    test("Node.distanceTo, Berlin - Tokio") {
    ...
    test("Node.distanceTo, Flinders Peak - Buninyong") {
    ...
    test("Node.distanceTo, nearly antipodal points 1") {
    ...
    test("Node.distanceTo, nearly antipodal points 2") {
        val node1 = new Node(1, new GeographicCoordinate(0, 0))
